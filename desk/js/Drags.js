(function(){
		...
		var newDelData = JSON.parse(localStorage.getItem("delDatas")) || []; //存放删除的数据
		//拖拽的构造函数
		function Drag(){}
		//元素拖拽三要素：
			...
			boxDom.onmousedown = function(e){
				...
				document.onmousemove = function(e){
					if(mark){
						...
						if(isPz){
							if(parentDom.id !== "recycleBin-panel")
				...
				document.onmouseup = function(){
					...
					if(isPz){
						...
						newDelData = JSON.parse(localStorage.getItem("delDatas")) || [];
						var id = parentDom.id;
						...
						newDelData = newDelData.concat(delData[0]);
						localStorage.setItem("delDatas",JSON.stringify(newDelData))
						if(newDelData.length!==0) $("garbage").querySelector('.icon').classList.add("garbagefull");
